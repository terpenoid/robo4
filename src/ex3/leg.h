...
class Leg {
  ...
  public:
    ...
    int filteredServoAngle(int servo, int angle) {
      switch (servo) {
        case 0: 
          if (angle < 0) angle = 0;
    ...
    int getRealAngleForServo (int servo, int angle) {
      ...
      angle = filteredServoAngle(servo, angle);
    ...
    void moveServo (int servo, int angle) {
      ...
      int pin[3] = {coxaPin, femurPin, tibiaPin};
      
      pwm.setPWM(pin[servo], 0, angleToPulse(real));
    }
